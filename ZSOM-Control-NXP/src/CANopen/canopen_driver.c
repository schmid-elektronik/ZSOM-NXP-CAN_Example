...
/* CAN Callback Function for SDO */
F_CAN_OPEN_RX_CB canOpen_RX_Callback;
...
** Returned value:		None
*****************************************************************************/
void CANopenInit(int canBaudRate)
{
	...
	baudrateCalculate(canBaudRate, ClkInitTable);

	/* Initialize the CAN controller */
	LPC_CCAN_API->init_can(&ClkInitTable[0], 1);

	/* Configure the CAN callback functions */
	LPC_CCAN_API->config_calb(&callbacks);

	/* Initialize CANopen handler */
	canOpenCfg.od_const_num = numberConstObjectDirEntries;
	canOpenCfg.od_num = numberObjectDirEntries;
	LPC_CCAN_API->config_canopen((CCAN_CANOPENCFG_T *)&canOpenCfg);
...
void CAN_RX(uint8_t msg_obj_num)
{
	...
	else if (canOpen_RX_Callback)
	{
		canOpen_RX_Callback(&CANopen_Msg_Obj);
	}
	return;
}
...
{
	static CCAN_MSG_OBJ_T msg_NMT_TX =
	{
			0x00,						/* mode_id */
			0x00,						/* mask */
			{
	...
	LPC_CCAN_API->can_transmit(&msg_NMT_TX);
}
...
{
	static CCAN_MSG_OBJ_T msg_HeartbeatTX =
	{
	...
	LPC_CCAN_API->can_transmit(&msg_HeartbeatTX);
}
...
void CANopen_SDOS_Find_Buffer(uint16_t index, uint8_t subindex, SDOS_Buffer_t** SDOS_Buffer, uint8_t* access_type)
{
	...
	*SDOS_Buffer = NULL;
	for(i=0; i< numberObjectDirEntries; i++)
	{
		/* scan all items in OD for the specified index/subindex  */
		if(objectDir[i].index == index && objectDir[i].subindex == subindex)
		{
			//OD entry exists, now load pointers
			*SDOS_Buffer = (SDOS_Buffer_t*)objectDir[i].val;
			*access_type = objectDir[i].entrytype_len;
			break;
		}
	}
}
...
uint8_t CANopen_SDOC_Exp_Read(uint8_t node_id, uint16_t index, uint8_t subindex, uint8_t* data, uint8_t* valid_data_bytes)
{
	...
	if(CANopen_SDOC_State == CANopen_SDOC_Succes || CANopen_SDOC_State == CANopen_SDOC_Fail)
	{
		...
		CANopen_Timeout = canOpenTimeoutVal;
		return 1;
	}
	CANopen_SDOC_State = CANopen_SDOC_Fail;
	return 0;
...
{
	if (CANopen_NMT_MyState == NMT_STATE_OPERATIONAL)
	{
		LPC_CCAN_API->can_transmit(msg_TX);
	}
}

void setCANOpen_RX_Callback(F_CAN_OPEN_RX_CB cb, uint8_t msgobj, uint32_t  mode_id)
{
    CCAN_MSG_OBJ_T msg_RX;
    msg_RX.msgobj = msgobj;
    msg_RX.mode_id = mode_id;
    msg_RX.mask = 0x7FF;

	LPC_CCAN_API->config_rxmsgobj(&msg_RX);
	canOpen_RX_Callback = cb;
}
